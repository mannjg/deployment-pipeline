...
#   - templates/ directory reset to main branch (all CUE definitions)
...
reset_env_via_mr() {
    ...
    # 2a. Get templates/ files from main
    local encoded_project=$(echo "$DEPLOYMENTS_REPO_PATH" | sed 's/\//%2F/g')
    local tree_url="$GITLAB_URL_EXTERNAL/api/v4/projects/$encoded_project/repository/tree?ref=main&path=templates&recursive=true&per_page=100"
    ...
    log_info "  Prepared ${#files_to_commit[@]} files from templates/"
    ...
    # 2f. Delete seed-env.cue if it exists on the branch (seed template, only belongs on main)
    files_to_commit+=("seed-env.cue:delete")
...
Resets templates/, scripts/, Jenkinsfile, and env.cue from main.
...
main() {
    ...
    echo "  - templates/ directory reset from main"
    ...
    log_info "  - templates/ directory reset from main"
