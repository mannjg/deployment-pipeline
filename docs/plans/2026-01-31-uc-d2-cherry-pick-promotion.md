# UC-D2: Cherry-Pick Promotion Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement selective app promotion when multiple apps have changes, allowing operators to promote specific apps while holding others back.

**Architecture:** Add `--only-apps` filter to existing `promote-app-config.sh`, then create demo script that exercises selective promotion from dev→stage for example-app while holding postgres back.

**Tech Stack:** Bash, CUE, GitLab API (via gitlab-cli.sh), Jenkins CI, ArgoCD
...
│             │     Dev     │    Stage    │     Dev     │    Stage    │
├─────────────┼─────────────┼─────────────┼─────────────┼─────────────┤
│ example-app │ 1.0.40-abc  │ 1.0.39-def  │ 1.0.40-abc  │ 1.0.40-abc  │ ← PROMOTED
│ postgres    │ 17-alpine   │ 16-alpine   │ 17-alpine   │ 16-alpine   │ ← HELD BACK
└─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘
```

---

## Task 1: Add --only-apps Filter to promote-app-config.sh

**Files:**
- Modify: `k8s-deployments/scripts/promote-app-config.sh:45-78` (argument parsing)
- Modify: `k8s-deployments/scripts/promote-app-config.sh:178-186` (app loop filtering)

**Step 1.1: Update argument parsing section**

In `k8s-deployments/scripts/promote-app-config.sh`, replace lines 45-78 with:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --only-apps <apps>  Comma-separated list of apps to promote (CUE identifiers)"
            echo "                      Example: --only-apps exampleApp,postgres"
            echo "                      If not specified, promotes all apps."
            echo ""
            echo "Example:"
            echo "  git checkout stage"
            echo "  $0 dev stage                           # Promote all apps"
            echo "  $0 dev stage --only-apps exampleApp    # Promote only exampleApp"
            exit 0
            ;;
        --only-apps)
            ONLY_APPS="$2"
...
**Step 1.2: Add filtering logic in app loop**

In the same file, find line 178 (`for APP in $APPS; do`) and add filtering immediately after the loop starts. Replace lines 178-186:

```bash
# Process each app
for APP in $APPS; do
    # Filter by --only-apps if specified
    if [[ -n "$ONLY_APPS" ]]; then
...
**Step 1.3: Update summary to show filter status**

Find line 251-256 (summary section) and update to:
...
log_info "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
[[ -n "$ONLY_APPS" ]] && log_info "  App filter:     $ONLY_APPS"
...
**Step 1.4: Test the modification manually**

Run from k8s-deployments directory (on stage branch):
```bash
# Test help shows new option
./scripts/promote-app-config.sh --help

# Expected: Shows --only-apps in usage
```

**Step 1.5: Commit the change**

```bash
cd /home/jmann/git/mannjg/deployment-pipeline/k8s-deployments
git add scripts/promote-app-config.sh
git commit -m "feat(promote): add --only-apps filter for selective promotion

Enables cherry-pick promotion by filtering which apps to promote.
When --only-apps is specified, only listed apps are promoted;
others are skipped. Backward compatible (no filter = all apps).

Implements UC-D2 requirement.
...
## Task 2: Create Demo Script Skeleton

**Files:**
- Create: `scripts/demo/demo-uc-d2-cherry-pick.sh`

**Step 2.1: Create the demo script with full implementation**
...
# Get image from GitLab env.cue for an app
get_gitlab_image() {
    local env="$1"
    local app_cue="$2"
    local env_cue
    env_cue=$(get_file_from_branch "$env" "env.cue")
    echo "$env_cue" | grep -A5 "${env}: ${app_cue}:" | grep 'image:' | head -1 | sed 's/.*image: "\([^"]*\)".*/\1/'
}
...
git clone --quiet --branch "$FEATURE_BRANCH" \
    "$(git remote get-url origin 2>/dev/null || echo "https://gitlab.jmann.local/p2c/k8s-deployments.git")" \
    "$WORK_DIR" 2>/dev/null || {
    # Fallback: use GitLab API to get files
    demo_info "Using GitLab API for file operations..."
}
...
git clone --quiet --branch "$CLEANUP_BRANCH" \
    "$(git remote get-url origin 2>/dev/null || echo "https://gitlab.jmann.local/p2c/k8s-deployments.git")" \
    "$CLEANUP_DIR" 2>/dev/null
...
```bash
chmod +x /home/jmann/git/mannjg/deployment-pipeline/scripts/demo/demo-uc-d2-cherry-pick.sh
```

**Step 2.3: Commit the demo script**

```bash
cd /home/jmann/git/mannjg/deployment-pipeline
git add scripts/demo/demo-uc-d2-cherry-pick.sh
git commit -m "feat(demo): add UC-D2 cherry-pick promotion demo

Demonstrates selective app promotion using --only-apps filter.
Promotes example-app from dev to stage while holding postgres back.
...
## Task 3: Add UC-D2 to Validation Suite

**Files:**
- Modify: `scripts/demo/run-all-demos.sh:53` (add UC-D2 entry)

**Step 3.1: Add UC-D2 to DEMO_ORDER array**

In `scripts/demo/run-all-demos.sh`, find line 53 (after UC-D5) and add UC-D2:

```bash
    "UC-D5:demo-uc-d5-skip-env.sh:Skip environment (dev→prod direct):dev,prod"
    "UC-D2:demo-uc-d2-cherry-pick.sh:Cherry-pick promotion (selective app):dev,stage"
)
```
...
git commit -m "feat(demo): add UC-D2 to validation suite
...
**Files:**
- Modify: `docs/USE_CASES.md:430` (UC-D2 status row)

**Step 4.1: Update implementation status**

In `docs/USE_CASES.md`, find the UC-D2 row (around line 430) and update from:
...
```markdown
| UC-D2 | Cherry-pick promotion (multi-app) | ✅ | ✅ | 🔲 | `uc-d2-cherry-pick` | Selective promotion via --only-apps filter |
```

**Step 4.2: Commit the change**

```bash
cd /home/jmann/git/mannjg/deployment-pipeline
git add docs/USE_CASES.md
git commit -m "docs: update UC-D2 implementation status
...
## Task 5: Sync to GitLab and Run Validation
...
**Step 5.2: Sync k8s-deployments to GitLab**

```bash
./scripts/04-operations/sync-to-gitlab.sh
```

**Step 5.3: Reset demo state**

```bash
./scripts/03-pipelines/reset-demo-state.sh
```

**Step 5.4: Run UC-D2 demo in isolation**

```bash
./scripts/demo/run-all-demos.sh UC-D2
```

Expected output: Demo completes with all verifications passing.

**Step 5.5: Run full validation suite**

```bash
./scripts/demo/run-all-demos.sh
```

Expected output: All demos pass, no regressions.

---

## Task 6: Mark UC-D2 as Pipeline Verified

**Step 6.1: Update USE_CASES.md with verification date**

After successful validation, update the UC-D2 row to:

```markdown
| UC-D2 | Cherry-pick promotion (multi-app) | ✅ | ✅ | ✅ | `uc-d2-cherry-pick` | Pipeline verified YYYY-MM-DD; Selective promotion via --only-apps filter |
```

**Step 6.2: Commit final status**

```bash
cd /home/jmann/git/mannjg/deployment-pipeline
git add docs/USE_CASES.md
git commit -m "docs: mark UC-D2 as Pipeline Verified
...
- [ ] Task 1: Add --only-apps filter to promote-app-config.sh
- [ ] Task 2: Create demo-uc-d2-cherry-pick.sh
- [ ] Task 3: Add UC-D2 to run-all-demos.sh
- [ ] Task 4: Update USE_CASES.md status
- [ ] Task 5: Sync to GitLab and run validation
- [ ] Task 6: Mark as Pipeline Verified
