...
After manual configuration above, you'll create this pipeline job:

1. example-app-ci (THE ONLY PIPELINE YOU NEED)
   - Type: Pipeline (Jenkinsfile from SCM)
   - SCM: Git
   - Repository URL: http://gitlab.gitlab.svc.cluster.local/example/example-app.git
     ⚠️ IMPORTANT: Use internal cluster DNS, NOT http://gitlab.local
     ⚠️ IMPORTANT: Namespace is 'example', NOT 'root'
   - Credentials: gitlab-credentials
   - Branch: */main
   - Script Path: Jenkinsfile
   - Build Triggers (optional, skip for initial testing):
     ✓ Build when a change is pushed to GitLab
     ✓ Accepted Merge Request Events

   This pipeline handles EVERYTHING:
   - Builds and tests the application
   - Publishes Docker image to Nexus
   - Updates k8s-deployments dev branch
   - Creates promotion MRs automatically

OUTDATED - IGNORE THESE (kept for reference only):
~~2. deployment-updater~~ - NOT NEEDED (handled in Jenkinsfile)
~~3. environment-promoter~~ - NOT NEEDED (handled via GitLab MRs)

========================================
Webhook Configuration (Done Later)
========================================

⚠️ OPTIONAL - Can skip initially and trigger builds manually

After creating the example-app project in GitLab:
1. Go to: http://gitlab.local/example/example-app → Settings → Webhooks
2. URL: http://jenkins.jenkins.svc.cluster.local:8080/project/example-app-ci
   ⚠️ Use internal cluster DNS for reliability
3. Secret Token: (leave empty for now)
4. Trigger: Push events (branch: main), Merge request events
5. SSL verification: Disable (local dev)

Note: For initial testing, you can trigger builds manually in Jenkins UI
