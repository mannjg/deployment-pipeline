...
exampleApp: core.#App & {
	...
	// Use CUE default syntax (*value | type) for fields that environments can override.
	// Env overrides with concrete values: (*10 | int) & 30 = 30
	appConfig: {
		deployment: {
			readinessProbe: {
				timeoutSeconds: *10 | int // App default, env can override with concrete
			}
		}
